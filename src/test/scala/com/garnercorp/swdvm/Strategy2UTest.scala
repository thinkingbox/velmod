...
class Strategy2UTest extends FunSuite with Matchers with EasyMockSugar {
  test("strategy2: given a previous task run with a certain velocity & tech debt, " +
  ...
  test("strategy2: when time > work required / velocity and we are on time or early, " +
  ...
  test("strategy2: when time > work required / velocity, but there's no tech debt, " +
  ...
  test("strategy2: when time > work required / velocity and we are late, " +
  ...
  test("strategy2: tech debt cannot become negative and velocity cannot increase the ideal value (# of programmers)") {
  ...
  test("strategy2: actual work cannot be less than 25% of work required") {
    val taskRun = Strategy2(TaskRun(2, 0),
                            Task.empty,
                            desiredTime = 1.0,
                            workRequired = 10.0,
  ...
  test("strategy2: velocity cannot become negative") {
    val taskRun = Strategy2(TaskRun(2, 0),
                            Task.empty,
                            desiredTime = 1.0,
                            workRequired = 1000.0,
  ...
  test("when running strategy2, if previous run reaches velocity 0 the next run is dead") {
  ...
  test("when running strategy2, next run uses snapshot data from previous run, without recalculating") {
  ...
  test("strategy2: if we are early with the previous task and the next one requires more work than desired, " +
  ...
  test("strategy2: even if we were early before, constantly underestimating will eventually lead to tech debt, " +
  ...
  test("strategy2: when balance is 0 if next task is desired to be done in less than 25% of time, " +
  ...
  test("strategy2: even when balance is > 0 if next task is desired to be done in less than 25% of time, " +
