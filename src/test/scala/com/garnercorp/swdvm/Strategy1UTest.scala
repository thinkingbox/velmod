...
class Strategy1UTest extends FunSuite with Matchers {
  ...
  test("time, work actual & velocity are calculated directly from work required & programmers") {
    Strategy.time shouldBe 2.0
    Strategy.workActual shouldBe 6.0
    Strategy.velocity shouldBe 3
  }
  test("there is never accumulation of tech debt") {
    Strategy.techDebt shouldBe 0.0
  }
  test("run ignores previous tasks and takes a snapshot with factor and random variation") {
